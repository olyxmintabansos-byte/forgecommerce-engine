...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-950/85 backdrop-blur-md border-b border-slate-800">
      ...
      <div className="bg-gradient-to-r from-cyan-950 via-slate-900 to-indigo-950 px-4 py-1.5 text-center text-[11px] font-medium text-slate-300 border-b border-slate-800/60 flex items-center justify-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 max-w-md hidden md:block">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Cari keyboard mekanik, standing desk, audio studio..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 bg-slate-900/80 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              if (confirm('Reset ulang data katalog dan keranjang ke kondisi awal?')) {
          ...
          <button
            onClick={onOpenCart}
            className="relative px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-slate-950 font-bold text-xs flex items-center gap-2 shadow-lg shadow-cyan-500/20 transition-all active:scale-95"
          >
